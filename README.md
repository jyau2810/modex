...
- API Key 身份：支持用 API Key 新增独立身份，自动读取账号名（可用时），不查询额度，并可为该身份配置可选 Base URL。
...
浏览器登录身份切换时会同步对应身份目录下的 Codex `auth.json`。API Key 身份会保存为独立身份，切换时同步 API Key 形式的 `auth.json`，并按需写入或清理该身份的 `openai_base_url` 配置；API Key 身份不进行额度查询，列表中仅显示 API Key 可用状态；项目配置和本地会话不会被覆盖。
